    ...
    for attempt in range(MAX_ATTEMPTS):
        ...
        try:
            ...
            if attempt < MAX_ATTEMPTS - 1:
                print(
                    f"[cue-kit] whisper HTTP {exc.code} — retrying in {delay:.1f}s "
            ...
            if attempt < MAX_ATTEMPTS - 1:
                ...
                print(
                    f"[cue-kit] whisper network error ({type(exc).__name__}: {exc}) — "
...
def transcribe_video(
    ...
    print(f"[cue-kit] audio: {size_kb:.0f} kB — uploading to {backend} Whisper…", file=sys.stderr)
