...
Auto-fps targets a frame budget, not a fixed rate — token cost downstream
scales with frame count, so budget-by-duration keeps short videos dense and
long videos capped. When a user-specified range is passed, focused-mode
budgets denser (they are zooming in for detail).
"""
...
def extract(
    ...
    # -ss before -i = fast seek (keyframe-snap, fine for preview frames).
